...
import {
  ...
  RefreshCw, Shield, Copy, Check, ChevronRight, Zap, Sparkles,
...
export default function ContentMachineUI({ onBack }: ContentMachineUIProps) {
  ...
  // ─── Lógica de integración con ALEGR-IA OS ───────────────────────────────
  
  const callAlegriaGenerate = async (params: { strategy: string, format: string, topic: string, mode: string }) => {
  ...
  const handleIdeas = async () => {
    ...
    setIdeas([]);
    try {
      const data = await callAlegriaGenerate({
        strategy,
        format, // Se ignora en modo ideas pero el API lo pide
        topic: '',
        mode: 'ideas'
      });
      
      const raw = data.response;
      // El backend ya debería devolver JSON limpio o el string del array
  ...
  const strategies = [
    { id: 'standard', name: 'Estándar', desc: 'Brand Voice activa. Rápido y preciso.', icon: <Zap size={16} /> },
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
        ...
        <div className="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-10">
                  ...
                  <div
                    ...
                    className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center gap-4 ${
                      strategy === opt.id
                        ...
                        : 'bg-[#1f2c34]/50 border-white/5 hover:bg-white/5'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`p-4 rounded-2xl border transition-all flex items-center gap-3 ${
                      format === opt.id
                        ...
                        : 'bg-[#1f2c34] border-white/5 hover:bg-white/5'
                    }`}
                  >
                    <div className={format === opt.id ? 'text-white' : 'text-gray-400'}>{opt.icon}</div>
                    <span className={`text-[11px] font-bold uppercase tracking-tight ${format === opt.id ? 'text-white' : 'text-gray-400'}`}>
                      {opt.name}
                    </span>
                  </button>
                ))}
          ...
          <div className="flex flex-col">
            <div className="flex-1 bg-[#1f2c34]/20 border border-white/5 rounded-[32px] flex flex-col overflow-hidden relative">
              ...
              {!output && !generating && (
                <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
                  ...
                  <h3 className="text-base font-bold text-white/60 mb-2">Output Hub</h3>
                  <p className="text-[10px] text-gray-600 uppercase tracking-widest leading-relaxed max-w-[200px]">
                    Configurá estrategia, formato y tema — la IA híbrida hace el resto.
                  </p>
                </div>
              )}
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-12 gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[10px] text-gray-600 mt-1">
                      Estrategia: {strategies.find(s => s.id === strategy)?.name} · {formatLabels[format]}
                    </p>
                  </div>
                </div>
              )}
              ...
              {output && !generating && (
                <div className="flex flex-col h-full">
                  ...
                  <div className="flex items-center justify-between px-5 py-3 border-b border-white/5">
                    ...
                    <button
                      onClick={handleCopy}
                      className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest transition-colors text-gray-500 hover:text-white"
                    >
                      {copied ? <><Check size={12} className="text-green-400" /><span className="text-green-400">Copiado</span></> : <><Copy size={12} />Copiar</>}
                    </button>
                  </div>

                  {/* Texto generado */}
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-5">
                    <pre className="text-[12px] text-gray-200 whitespace-pre-wrap leading-relaxed font-mono">
                      {output}
                    </pre>
                  </div>
